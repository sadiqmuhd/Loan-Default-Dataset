...
Every risk response carries the model version, the request id and the assumption
set behind any loss figure. The original API returned
``{prediction, probability, risk_level}`` with no provenance at all, which makes
a credit decision untraceable and therefore unauditable.
"""
...
class ReadinessResponse(BaseModel):
    """Readiness verifies the model *works*, not merely that a file exists.

    The original ``/health`` called ``os.path.exists`` on an artifact that could
    not actually be unpickled, and reported healthy.
    """
