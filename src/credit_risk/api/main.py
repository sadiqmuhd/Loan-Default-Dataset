...
``service.py``, risk methodology in ``credit_risk.risk``, and HTTP concerns in
...
from credit_risk.api.dependencies import load_model_state
from credit_risk.api.errors import register_exception_handlers
from credit_risk.api.middleware import RequestContextMiddleware
from credit_risk.api.routers import health, model, portfolio, risk
...
Protected characteristics (gender, age) are **not collected by this API** and are
excluded from the model under ECOA / Regulation B. Measured cost of exclusion:
4 basis points of ROC-AUC.
