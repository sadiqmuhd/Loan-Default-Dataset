...
def compute_loss_components(
    ...
    cfg = policy or load_risk_policy()
    has_collateral = property_value is not None and np.isfinite(property_value)
    result = expected_loss(
        [pd_value],
        [loan_amount],
        [property_value] if has_collateral else None,
        cfg,
    )
    return LossComponents(
        ...
        collateral_value=float(property_value) if has_collateral else None,
        lgd_method="collateral_proxy" if has_collateral else "fallback_flat_rate",
    )
