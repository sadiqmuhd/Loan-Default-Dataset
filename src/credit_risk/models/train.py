...
from credit_risk.config import get_settings, load_model_config
...
from credit_risk.models.registry import ModelRegistry, build_metadata, new_version_string
...
def train(
    *,
    ...
    if report.roc_auc > ceiling:
        raise LeakageGuardError(
            f"Test ROC-AUC {report.roc_auc:.4f} exceeds the plausible ceiling {ceiling}. "
            "A leaking feature has almost certainly been reintroduced. "
            "The original model scored 1.0000 because Interest_rate_spread.isna() "
            "equalled the target for 148,670 of 148,670 rows."
        )
