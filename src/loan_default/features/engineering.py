"""Feature engineering, used by both training and serving.

This is the only definition of these transforms; ``test_train_serve_parity``
asserts the training and inference paths produce identical output.

Note there are no missingness indicators here, and there must not be. On this
dataset the missingness of several pricing fields encodes the target almost
perfectly, so deriving features from it leaks. ``test_no_leakage`` enforces
that. See docs/LEAKAGE_INVESTIGATION.md.
"""
...
from loan_default.config import load_model_config
