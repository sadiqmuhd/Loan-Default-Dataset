...
from loan_default.api.schemas.requests import (
    BatchAssessmentRequest,
    LoanApplication,
    validate_application,
)
from loan_default.api.schemas.responses import (
...
def assess(
    ...
    payload = application.model_dump(by_alias=True)  # type: ignore[attr-defined]
...
def assess_batch(
    ...
    # Validate row by row so one malformed application does not cost the caller
    # the whole batch. Invalid rows are reported in place, at their index.
    valid: list[dict] = []
    positions: list[int] = []
    failures: dict[int, str] = {}
    for index, payload in enumerate(request.applications):
        cleaned, error = validate_application(payload)
        if cleaned is None:
            failures[index] = error or "invalid application"
        else:
